...
def find_token(text):
    ...
    while i < len(tokens):
        ...
        #     i += 1
            ...
            # elif re.match(r'[-+*/=.().,:<>]|\*\*', tken):
            # Tokens.append(token(tken, Token_type.OPERATOR))
        # elif re.match(r'[*|\/]', tokens[i]):
        # Tokens.append(token(tokens[i], Token_type.MULTOP))
        # i += 1
        # elif re.match(r'\+|-', tokens[i]):
        # Tokens.append(token(tokens[i], Token_type.ADDOP))
        # i += 1
        # elif re.match( r'>|<|<=|>=', tokens[i]):
        # Tokens.append(token(tokens[i], Token_type.RELOP))
        # i += 1
        # elif re.match(r'==|//=', tokens[i]):
        # Tokens.append(token(tokens[i], Token_type.EQUOP))
        # i += 1
...
canvas1.pack()
background_image = ImageTk.PhotoImage(Image.open("binary3.jpg"))
background_label = tk.Label(root, image=background_image)
background_label.place(x=0, y=0, relwidth=1, relheight=1)


# Create a list of image URLs or file paths
image_urls =  ["identifiers.png", "constants2.png", "reservedwords.png","strings.png"]
...
def load_images():
    ...
    # Calculate the number of columns needed based on the number of images
    num_columns = len(image_urls)

    # Load the current image
    current_image = Image.open(image_urls[current_image_index])
    current_image = current_image.resize((800, 800), Image.ANTIALIAS)
    photo = ImageTk.PhotoImage(current_image)

    label3 = tk.Label(image_container, image=photo)
    label3.image = photo
    label3.grid(row=0, column=current_image_index)

    # Update the current image index for the next click
    current_image_index = (current_image_index + 1) % num_columns

# Create a button to load images
button3 = tk.Button(root, text="show DFA", command=load_images)
button3.pack()
...
def generate_dfa(lex):
    ...
    identifier_dfa.node('accept', shape='doublecircle',fillcolor='red')
...
def Scan_scanner():
    ...
    button = tk.Button(frame, text="Click", command=lambda w=x1: generate_dfa(w))
